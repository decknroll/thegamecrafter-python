...
from thegamecrafter import api, responses
...
class TheGameCrafterTestCase(TestCase):
    def setUp(self):
        patcher = patch('thegamecrafter.requests.request')
        self.addCleanup(patcher.stop)
        self.request = patcher.start()

        self.request.return_value = StubResponse(200, {})

        self.client = api.TheGameCrafter()
    ...
    def test_get_invalid_resource(self):
        with self.assertRaises(api.TheGameCrafterError):
    ...
    # FIXME update Shipwire tests for TheGameCrafter API
    ...
    #                           responses.TheGameCrafterResponse)
